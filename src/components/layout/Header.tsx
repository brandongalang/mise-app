...
import { useSession, AVATAR_COLORS } from "@/contexts/SessionContext";
import Avatar from "@/components/auth/Avatar";
import { Drawer } from "vaul";
import { LogOut, Settings, ChevronDown } from "lucide-react";

export default function Header() {
  const { activeProfile, profiles, selectProfile, exitProfile } = useSession();
       ...
       <Drawer.Root>
          <Drawer.Trigger asChild>
            <button className="flex items-center gap-2 p-1 pr-2 rounded-full hover:bg-black/5 transition-colors focus:outline-none">
               <Avatar colorClass={AVATAR_COLORS[activeProfile.avatar_color]} name={activeProfile.display_name} size="sm" />
                       ...
                       {profiles.map((profile) => (
                         ...
                         >
                            <div className={`relative transition-transform duration-200 ${activeProfile.id === profile.id ? 'ring-2 ring-[var(--color-terracotta)] ring-offset-2 rounded-full scale-105' : 'group-hover:scale-105'}`}>
                               <Avatar colorClass={AVATAR_COLORS[profile.avatar_color]} name={profile.display_name} size="md" />
                            </div>
